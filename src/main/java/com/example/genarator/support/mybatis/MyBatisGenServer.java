package com.example.genarator.support.mybatis;
...
import com.example.genarator.metadata.factory.impl.MyBatisCodeMetaDataFactoryImpl;
 ...
 * <pre>
 * @author hexian
 * @description MyBatis 代码生成器 (自定义)
 * </pre>
 */ 
public class MyBatisGenServer implements CodeGenServer {
	...
	public MyBatisGenServer(DBConfig dbConfig, CodeConfig codeConfig) {
	...
	private void table2code(List<TableInfo> tbInfoList) {
		if(tbInfoList!=null && !tbInfoList.isEmpty()) {
			CodeMetaDataFactory facorty = MyBatisCodeMetaDataFactoryImpl.getInstance(codeConfig);
	...
	private FileInfo generatorMapperJava(TableInfo tbInfo, CodeMetaData codeMetaData) {
		...
		bodyBuilder.append("public interface "+mapperClassName+" {"+separator+separator);
		...
		if(primaryKeys.size()>0) {
			...
			// 字段名称
			String fieldName = CamelizeKit.underLine2Camelize(columnName.toLowerCase());
	...
	private FileInfo generatorService(TableInfo tbInfo, CodeMetaData codeMetaData) {
		...
		content.append("	public "+className+" save("+className+" "+entityLowerClassName+");"+separator+separator);
		...
		// 保存或更新
		content.append("	/** 根据实体对象删除  **/ "+separator);
		content.append("	public void saveOrUpdate("+className+" "+entityLowerClassName+"); " + separator+separator);
